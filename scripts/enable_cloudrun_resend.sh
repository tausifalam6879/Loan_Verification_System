...
store_secret_version verification-resend-api-key \
  "Paste Render RESEND_API_KEY (hidden), then press Enter: "
store_secret_version verification-mail-from \
  "Paste Render APP_MAIL_FROM (hidden), then press Enter: "

PROJECT_NUMBER="$(gcloud projects describe "$PROJECT_ID" --format='value(projectNumber)')"
RUNTIME_SERVICE_ACCOUNT="${PROJECT_NUMBER}-compute@developer.gserviceaccount.com"
for secret_name in verification-resend-api-key verification-mail-from; do
...
gcloud run services update "$API_SERVICE" \
  --project="$PROJECT_ID" \
  --region="$REGION" \
  --update-env-vars="APP_MAIL_ENABLED=true,APP_MAIL_PROVIDER=resend" \
  --update-secrets="RESEND_API_KEY=verification-resend-api-key:latest,APP_MAIL_FROM=verification-mail-from:latest" \
  --quiet

API_URL="$(gcloud run services describe "$API_SERVICE" --project="$PROJECT_ID" --region="$REGION" --format='value(status.url)')"
echo
echo "RESEND_CONFIGURATION_READY"
