...
export default function ExpenseSummaryPanels({ expenses = [], budgets = {} }) {
  ...
  return <Box sx={{ display: "grid", gridTemplateColumns: { xs: "minmax(0, 1fr)", lg: "1fr 1fr" }, gap: 2.5, mt: 2.5 }}>
    <Card elevation={0} sx={panel}>
      ...
      <Box role="region" aria-label="Recent expense transactions" tabIndex={0} sx={{ overflowX: "auto" }}><Box component="table" sx={{ minWidth: 480, width: "100%", borderCollapse: "collapse", fontSize: 13, "& th": { textAlign: "left", bgcolor: "#f5f3ff", color: "#637099", py: 1 }, "& td": { py: 1.5, borderBottom: "1px solid #eef1fb" }, "& th, & td": { px: 1 } }}>
    ...
    <Card elevation={0} sx={panel}>
      ...
      <Box sx={{ maxHeight: 170, overflowY: "auto" }}>{rows.map((row, index) => {
        ...
        return <Box key={row.category} sx={{ display: "grid", gridTemplateColumns: { xs: "minmax(0, 1fr) minmax(0, 1fr)", md: "110px minmax(60px,1fr) 42px 125px" }, gap: 1.2, alignItems: "center", py: 1.1 }}><Typography variant="body2" sx={{ textTransform: "capitalize" }}>{row.category}</Typography><LinearProgress variant="determinate" value={Math.min(100, percent)} aria-label={row.category + " budget usage"} sx={{ height: 12, borderRadius: 8, bgcolor: "#edf1fa", "& .MuiLinearProgress-bar": { bgcolor: color, borderRadius: 8 } }} /><Typography variant="caption" sx={{ fontWeight: 800 }}>{percent}%</Typography><Typography variant="caption" color="text.secondary">{money(row.spent)} / {row.limit.toLocaleString("en-IN")}</Typography></Box>;
      })}</Box>
